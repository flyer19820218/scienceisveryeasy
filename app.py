...
if selected_season == "第一季：化學大聯盟 ⚾":
    ...
    with st.expander("🔥 EP06 | 球場的五大無名英雄與石灰三兄弟變身大秀", expanded=True):
        ...
        st.markdown("- **醫療團隊的最愛**：硫酸鈣 ($CaSO_{4}$)，微溶於水的白色石膏。")
        st.markdown("- **石灰三兄弟變身閉環**：灰石 ($CaCO_{3}$) 加熱 $\\rightarrow$ 生石灰 ($CaO$) 加水 $\\rightarrow$ 熟石灰 ($Ca(OH)_{2}$) 吹入二氧化碳 $\\rightarrow$ 變回白色混濁的灰石！")
        st.markdown("- **最狂蘇打兄弟檔**：硬漢哥哥「碳酸鈉」($Na_{2}CO_{3}$) 是鹼性洗滌鹼；怕熱弟弟「碳酸氫鈉」($NaHCO_{3}$) 則是弱鹼性小蘇打，遇熱會吐出二氧化碳來滅火跟做麵包！")
    ...
    with st.expander("🌟 EP02 | 狂轟猛炸的「酸」球隊強勢踢館", expanded=False):
        ...
        st.markdown("**📌 戰術板亮點：**")
        st.markdown("- **三大王牌先發**：工業之母「硫酸」($H_{2}SO_{4}$) 的恐怖脫水性、穿微黃球衣的「鹽酸」($HCl$)，以及見光死的棕色瓶殺手「硝酸」($HNO_{3}$)。")
