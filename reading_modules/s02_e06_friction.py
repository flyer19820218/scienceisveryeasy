...
def render_reading_and_quiz():
    ...
    # ==========================================
    # 🕹️ HTML5 互動實驗室 (摩擦力模擬器)
    # ==========================================
    st.markdown("#### 🛠️ 互動實驗室：推動沉重的舞台音箱")
    st.info("👇 **請緩慢拖曳下方的「推力滑桿」，觀察【推力】與【摩擦力】的數值變化。注意突破「最大靜摩擦力」那一瞬間，阻力數值發生了什麼奇妙的變化！**")
    ...
    <div style="font-family: 'Helvetica Neue', sans-serif; padding: 20px; background-color: #f8fafc; border-radius: 12px; border: 2px solid #cbd5e1; box-shadow: 0 4px 6px rgba(0,0,0,0.05); text-align: center;">
        ...
        <div id="hint-msg" style="margin-top: 15px; font-size: 15px; color: #475569; font-weight: bold;">
            設定值：最大靜摩擦力 = 60 kgw / 動摩擦力 = 45 kgw
        </div>
    ...
    <script>
        ...
        function updatePhysics() {
            ...
            arrowPush.style.left = (boxPos - (F * 1.5)) + 'px'; // 箭頭尖端貼著箱子左側
            ...
            if (F == 0) {
                ...
                arrowFric.style.left = (boxPos + 80) + 'px'; // 箭頭尾端貼著箱子右側
                ...
                statusBadge.style.backgroundColor = "#dc2626"; // 紅色警告
                ...
                // 突破極限，滑動開始！摩擦力瞬間掉到動摩擦力
                isMoving = true;
                fricVal.innerText = KINETIC_FRIC;
                statusBadge.innerText = "滑動中 (動摩擦力)";
                statusBadge.style.backgroundColor = "#10b981"; // 綠色通行
    ...
    if st.button("⚖️ 提出雙重異議 (Double Objection!)", use_container_width=True):
        if not q1 or not q2:
            ...
            st.success("💥 雙重異議成立！(OBJECTION!)\n\n法官敲下法槌：「檢察官說得完全正確！從實驗室的數據也能清楚看見，突破最大靜摩擦力後，動摩擦力就會掉落並維持定值，絕不會隨速度增加。同時，接觸面積也無法改變摩擦力的宿命。辯方律師的常識完全違反了力學法則！」\n\n✅ 成功戳破所有謊言，審判勝利，成功捍衛了摩擦力的物理真相！")
            return True
        else:
            ...
            if not q2.startswith("B"):
                error_msg += "\n👉 **【關於動摩擦力】** 請回憶剛才的互動實驗，當推力超過 60kgw 進入滑動狀態後，摩擦力的數值是變大還是固定不變？"
            st.error(error_msg)
