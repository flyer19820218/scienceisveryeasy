...
if selected_season == "第一季：化學大聯盟 ⚾":
    ...
    # 🌟 NEW! 第九集：最新上架，預設展開 🌟
    with st.expander("🔥 EP09 | 化學平衡 (上)！沒有盡頭的延長賽與調度假象", expanded=True):
    ...
    with st.expander("🌟 EP01 | 超級新秀電解質", expanded=False):
        ...
        st.markdown("**📌 戰術板亮點：**")
        st.markdown("- **選秀標準 (電解質定義)**：必須要能「溶於水」且能「導電」的化合物，兩者缺一不可！例如固體食鹽不導電，但食鹽水可以！")
        st.markdown("- **阿瑞尼斯的解離暗號**：球員一下水就會自動拆解成帶正電與帶負電的「離子球員」，在水中自由奔跑，這就是溶液導電的秘密！")
        st.markdown("- **非電解質板凳球員**：酒精和蔗糖雖然能溶於水，但卻像乖寶寶一樣不解離、不帶電，所以無法導電！")
    ...
    for i in range(10, 11):
        with st.expander(f"🔒 EP{i:02d} | 精彩賽事轉播準備中...", expanded=False):
            st.write(f"第 {i} 集的大結局戰術正在牛棚熱身中，剪輯完畢就會馬上派上場！敬請期待。")
